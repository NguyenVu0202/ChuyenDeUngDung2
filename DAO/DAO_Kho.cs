...
namespace DAO
{
	public class DAO_Kho
	{
		...
		//public void CapNhatSanPhamTrongKho(string maKho, string maCH, string maSP, decimal soLuongCapNhat)
		//{
		//    using (var db = new DataBHXDataContext(DAODoiChuoiKetNoi.Instance.ThayDoiChuoiKetNoi()))
		//    {
		//        // Kiểm tra xem MaSP có tồn tại trong bảng SanPham không
		//        var sanPham = db.Khos.FirstOrDefault(sp => sp.MaSP == maSP);
		//        if (sanPham == null)
		//        {
		//            throw new Exception("Mã sản phẩm không tồn tại trong bảng SanPham.");
		//        }

		//        var khoEntry = db.Khos.FirstOrDefault(k => k.MaKho == maKho && k.MaCH == maCH && k.MaSP == maSP);

		//        if (khoEntry != null)
		//        {
		//            khoEntry.SoLuong += soLuongCapNhat;
		//        }
		//        else
		//        {
		//            var newKhoEntry = new Kho
		//            {
		//                MaKho = maKho,
		//                MaCH = maCH,
		//                MaSP = maSP,
		...
		//            };
		//            db.Khos.InsertOnSubmit(newKhoEntry);
		//        }
		...
		public bool Sua(Kho kho)
		{
			using (var db = new DataBHXDataContext(DAODoiChuoiKetNoi.Instance.ThayDoiChuoiKetNoi()))
			{
				var edit = db.Khos.SingleOrDefault(a => a.MaKho == kho.MaKho && a.MaCH == kho.MaCH && a.MaSP == kho.MaSP);

				if (edit != null)
				{
					edit.SoLuong = kho.SoLuong;
					db.SubmitChanges();
					return true;
				}
				else
